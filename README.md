- [​Экзаменационный билет №1](#экзаменационный-билет-1)
  ...
  - [​Дайте определение регистров общего назначения, перечислите их и приведите примеры их явного и неявного использования.](#дайте-определение-регистров-общего-назначения-перечислите-их-и-приведите-примеры-их-явного-и-неявного-использования)
    - [Явное использование](#явное-использование)
    - [Неявное использование](#неявное-использование)
...
- [​Экзаменационный билет №12](#экзаменационный-билет-12)
  - [​Перечислите операторы ассемблера.](#перечислите-операторы-ассемблера)
    ...
    - [Определение размера типа](#определение-размера-типа)
    - [Оператор `()` (круглые скобки).](#оператор--круглые-скобки)
    - [Побитовые операторы](#побитовые-операторы)
    - [`seg` и `offset`](#seg-и-offset)
...
```asm
mul BL  ; Умножение AL на BL. Результат будет храниться в AX
```
...
## ​Перечислите операторы ассемблера.
...
Сдвиг может быть использован для более быстрого деления или умножения на степени двойки. «Выдвинутый» байт помещается во флаг СЕ.
...
* `base[index]` - сложить базовый адрес со смещением.
* `mov ax,mas[si]` - пересылка слова по адресу mas+(si) в регистр ax

Переопределение/явное указание типа: 

`new_type ptr`

new_type: byte 1, word 2, dword 4, qword 8, tword 10. 

near и far - ближние и дальние указатели
```asm
d_wrd dd 0
...
...
`length`. Определяет количество элементов в нем (!).
```asm
Array dw 10 dup (0)
...
`size`. Определяет его размер в байтах (!).
```asm
Array dw 10 dup (0);
...
`type`. Определяет тип одного элемента (тип в ассемблере - это размер в байтах).
...
Часть выражения, заключенная в них, вычисляется в первую очередь.

**Унарный минус ставится перед отрицательным числом**.

### Побитовые операторы

`and`, `not`, `or`, `xor` (и, не, или, исключающее или).
```asm
...
Возвращают соответствующую часть своего аргумента (сегментную часть для seg и смещение для offset):
...
определяет, что выполнится первым, если нет скобок.
